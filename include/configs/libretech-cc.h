...
/*
 * Configuration for Libre Computer AML-S905X-CC
 *
 * Copyright (C) 2021 Da Xue
...
#define LC_BOOTMENU_ITEMS_ENV \
	...
	"bootmenu_6=Boot PXE=" \
		...
		"sleep 5; " \
		"$menucmd -1\0" \
	"bootmenu_7=Boot DHCP=" \
		"run bootcmd_dhcp; " \
		"echo \"DHCP Boot failed.\"; " \
		"sleep 5; " \
		"$menucmd -1\0" \
	"bootmenu_8=SD USB Drive Mode=" \
		"mmc list; " \
		"if mmc dev 0; then " \
		...
		"else " \
			"echo \"SD not detected.\"; " \
		"fi; " \
		"sleep 5; " \
		"$menucmd -1\0" \
	"bootmenu_9=eMMC USB Drive Mode=" \
		"mmc list; " \
		"if mmc dev 1; then " \
			"echo \"Press Control+C to end USB Drive Mode.\"; " \
			"ums 0 mmc 1; " \
		...
		"sleep 5; " \
		"$menucmd -1\0" \
	"bootmenu_10=Detect USB Devices=" \
		"if usb reset; then " \
